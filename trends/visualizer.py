...
def plot_trends(df, keywords, filename="trend_plot.png"):
    ...
    for keyword in keywords:
        if keyword in df.columns:
            plt.plot(df.index, df[keyword], label=keyword, marker="o")
...
def plot_bar_chart(df, keywords, filename="bar_chart.png"):
    ...
    sns.barplot(x=avg_interest.index, y=avg_interest.values, hue=avg_interest.index, palette="viridis", legend=False)
